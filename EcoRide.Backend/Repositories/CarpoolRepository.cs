...
public class CarpoolRepository : ICarpoolRepository
{
    ...
    public async Task<List<Carpool>> SearchAsync(SearchCarpoolDTO searchDto)
    {
        var query = _context.Carpools
            .Include(c => c.Vehicle)
                .ThenInclude(v => v.Brand)
            .Include(c => c.Driver)
            .Where(c => c.DepartureCity.ToLower() == searchDto.DepartureCity.ToLower() &&
                       c.ArrivalCity.ToLower() == searchDto.ArrivalCity.ToLower() &&
                       c.DepartureDate.Date == searchDto.DepartureDate.Date &&
                       c.AvailableSeats > 0 &&
                       c.Status == "Pending");

        // Optional filters
        if (searchDto.IsEcological.HasValue && searchDto.IsEcological.Value)
        {
            query = query.Where(c => c.Vehicle.EnergyType.ToLower() == "electric");
        }

        if (searchDto.MaxPrice.HasValue)
        {
            query = query.Where(c => c.PricePerPerson <= searchDto.MaxPrice.Value);
        }

        if (searchDto.MaxDurationMinutes.HasValue)
        {
            query = query.Where(c => c.EstimatedDurationMinutes <= searchDto.MaxDurationMinutes.Value);
        }
    ...
    public async Task<Dictionary<DateTime, float>> GetPlatformCreditsEarnedByDateAsync(DateTime startDate, DateTime endDate)
    {
        var participations = await _context.CarpoolParticipations
            .Include(p => p.Carpool)
            .Where(p => p.ParticipationDate >= startDate &&
                       p.ParticipationDate <= endDate &&
                       p.Status == "Validated")
